...
HANDLE=os.getenv("BSKY_HANDLE","mraeburn.link");PASSWORD=os.environ["BSKY_APP_PASSWORD"];DRY_RUN=os.getenv("DRY_RUN","true").lower()=="true";STATE_FILE=Path(os.getenv("STATE_FILE","state.json"));MAX_REPLIES=int(os.getenv("MAX_REPLIES_PER_RUN","2"));MAX_FOLLOWS=int(os.getenv("MAX_FOLLOWS_PER_RUN","2"));MAX_LIKES=int(os.getenv("MAX_LIKES_PER_RUN","3"));MAX_INBOUND=int(os.getenv("MAX_INBOUND_REPLIES_PER_RUN","2"));REPLY_LIMIT=int(os.getenv("BSKY_REPLY_CHAR_LIMIT","220"));GENERIC_OPENING=re.compile(r"^(agree(?:d)?|exactly|absolutely|indeed|fair point|good point|great point|spot on|true|yes)\b[\s,.:;!—-]*",re.I)
def state_load():
...
def complete_reply(prompt):
 for attempt in range(3):
  suffix=f"\nWrite ONE concise, complete response <= {REPLY_LIMIT} characters. Goal: genuine two-way professional conversation. Start directly with substance: NEVER open with Agree, Agreed, Exactly, Absolutely, Indeed, Fair point, Good point, Great point, Spot on, True, Yes, praise, or a restatement of the author's post. First address the CENTRAL CLAIM in the supplied post; only then introduce a technical angle if it materially sharpens that claim. Prefer one educated, specific distinction or mechanism + one natural question about reasoning, implementation, evidence, incentives, constraints or trade-offs. Avoid compressed slogan-like phrases and jargon piles. No generic 'Thoughts?'. 1-2 short sentences; one useful point. End naturally. Output only reply or NO_REPLY."
  if attempt:suffix+=" Previous attempt failed a style or length check. Make it materially shorter, substance-first and more natural."
...
def process_inbound(c,me,s,block):
 ...
 for n in notes:
  ...
  reply=complete_reply(f"Inbound {n.reason} from @{n.author.handle}: {txt}\nRelationship stage:{profile['stage']} score:{profile['score']}\nHistory:\n{history or '- none'}\nContinue the actual conversation. Address what they are principally saying before adding any adjacent expertise. For recurring/established contacts, build on prior reasoning rather than resetting context. Ask one informed follow-up where useful.")
  if reply:
...
def main():
 ...
 for _,__,p,txt,topic,profile in cands[:30]:
  ...
  if replies<MAX_REPLIES and p.uri not in replied and d.get("reply_worthy") is True:
   reply=complete_reply(f"Topic:{topic}\nPost by @{p.author.handle}: {txt}\nRelationship:{profile}\nHistory:\n{history or '- none'}\nEnter as a conversation, not a mini-essay. Engage the post's central claim directly. Add one educated distinction, mechanism or implication that genuinely develops that claim, then normally one concrete question. Do not pivot to an adjacent technical topic merely because Martin knows about it. If recurring/established, continue the relationship's existing thread of thought.")
   if reply:
