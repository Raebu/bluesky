...
HANDLE=os.getenv("BSKY_HANDLE","mraeburn.link");PASSWORD=os.environ["BSKY_APP_PASSWORD"];DRY_RUN=os.getenv("DRY_RUN","true").lower()=="true";STATE_FILE=Path(os.getenv("STATE_FILE","state.json"));MAX_REPLIES=int(os.getenv("MAX_REPLIES_PER_RUN","2"));MAX_FOLLOWS=int(os.getenv("MAX_FOLLOWS_PER_RUN","2"));MAX_LIKES=int(os.getenv("MAX_LIKES_PER_RUN","3"));MAX_INBOUND=int(os.getenv("MAX_INBOUND_REPLIES_PER_RUN","2"));REPLY_LIMIT=int(os.getenv("BSKY_REPLY_CHAR_LIMIT","220"))
def state_load():
...
def complete_reply(prompt):
 for attempt in range(3):
  suffix=f"\nWrite ONE concise, complete response <= {REPLY_LIMIT} characters. Goal: genuine two-way professional conversation. Prefer one educated, specific observation + one natural question about reasoning, implementation, evidence, incentives, constraints or trade-offs. No generic 'Thoughts?'. 1-2 short sentences; one useful point. End naturally. Output only reply or NO_REPLY."
  if attempt:suffix+=" Make it materially shorter while preserving the conversational opening."
...
def quality_decision(a,t,topic,profile):
 return ai_json(f"Topic:{topic}\nAuthor:@{a.handle}\nDisplay name:{getattr(a,'display_name','') or ''}\nRelationship:{json.dumps(profile)}\nPost:{t}\nReturn JSON only {{\"reply_worthy\":false,\"like\":false,\"follow\":false,\"reason\":\"\"}}. Strict professional reputation filter. Prefer substantive two-way dialogue and, all else equal, recurring/established relationships over strangers. Existing relationship never overrides poor content. Reject promotional bundles/courses, affiliate content, spam, aggregators, bait, low-information posts, suspicious accounts, rage bait and partisan political campaigning. like only credible relevant substance; follow only worthwhile ongoing sources/peers. Uncertain=all false.")
def process_inbound(c,me,s,block):
 ...
 for n in notes:
  ...
  reply=complete_reply(f"Inbound {n.reason} from @{n.author.handle}: {txt}\nRelationship stage:{profile['stage']} score:{profile['score']}\nHistory:\n{history or '- none'}\nContinue the actual conversation. For recurring/established contacts, build on prior reasoning rather than resetting context. Ask one informed follow-up where useful.")
  if reply:
...
def main():
 ...
 for _,__,p,txt,topic,profile in cands[:30]:
  ...
  if replies<MAX_REPLIES and p.uri not in replied and d.get("reply_worthy") is True:
   reply=complete_reply(f"Topic:{topic}\nPost by @{p.author.handle}: {txt}\nRelationship:{profile}\nHistory:\n{history or '- none'}\nEnter as a conversation, not a mini-essay. Add one educated observation and normally one concrete question. If recurring/established, continue the relationship's existing thread of thought.")
   if reply:
